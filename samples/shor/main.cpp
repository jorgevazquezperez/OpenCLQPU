...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        cl::Kernel shor(program, "shor", nullptr);
        shor.setArg(0, heap);
        shor.setArg(1, countsbuf);
        shor.setArg(2, &n_counts);

        queue.enqueueWriteBuffer(countsbuf, CL_TRUE, 0, sizeof(COUNT) * count_dim, counts);
        queue.enqueueNDRangeKernel(shor, cl::NDRange(1), cl::NDRange(1));
