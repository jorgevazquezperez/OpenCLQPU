...
int main(int argc, char *argv[]) {
    ...
    // Create the heap
    Sources src;
    src.push_back(get_source("../malloc/clIndexedQueue.cl"));
	src.push_back(get_source("../kernel_malloc.cl"));
    ...
    try {
        // I create the heap
        cl::Buffer heap(kma_create(device.get(), context.get(), queue.get(), program.get(), 127));
        cl::Kernel kernel(program, "test_malloc_outside", nullptr);
        
        unsigned int iter = 5;
        kernel.setArg(0, heap);
        kernel.setArg(1, iter);
