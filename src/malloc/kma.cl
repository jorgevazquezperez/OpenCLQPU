...
typedef struct clheap __global *HEAP;
...
void bzero(void __global *s, size_t n)
{
  char __global* c = (char __global*)s; // Can't work with void *s directly.
  size_t i;
  for (i = 0; i < n; ++i)
    c[i] = '\0';
}
...
void __global * 
calloc(__global struct clheap *heap, size_t nelem, size_t elsize)
{
    void __global*p;

    p = malloc (heap, nelem * elsize);
    if (p == 0)
        return (p);
...
void __global* memcpy(void __global *dst, void __global *src, size_t len)
 {
    size_t i;
...
void swap(void __global* v1, void __global* v2, int size, HEAP heap)
{
    // buffer is array of characters which will 
    // store element byte by byte
    char __global *buffer = (char __global *)malloc(heap, sizeof(char)*size);
